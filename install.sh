#!/usr/bin/env bash
# sni-relay installer & manager — English UI, rainbow-colored
# Source: https://github.com/skyboy610/sni-relay
#
# Color rules:
...
GH_REPO="${SNIRELAY_GH_REPO:-skyboy610/sni-relay}"
REPO_URL="${SNIRELAY_REPO_URL:-https://github.com/${GH_REPO}}"
REPO_BRANCH="${SNIRELAY_REPO_BRANCH:-main}"
# Optional personal access token (for private-repo release downloads).
GH_TOKEN="${SNIRELAY_GH_TOKEN:-${GH_TOKEN:-}}"
...
download_binary() {
    ...
    if ! curl_gh -o "${tmp}" "${url}"; then
        ...
        if [[ -z "${GH_TOKEN}" ]]; then
            info "If the repository is private, set SNIRELAY_GH_TOKEN to a PAT with repo read access and retry."
        fi
        info "If no release exists yet, push a git tag like 'v1.0.0' to trigger the build workflow,"
        info "or run 'SNIRELAY_FROM_SOURCE=1 sudo bash install.sh' to build from source."
        rm -f "${tmp}"
        return 1
    fi

    # Verify checksum when available (optional — release provides checksums.txt).
    local sums="/tmp/sni-relay-checksums.$$"
    local sums_url
    if [[ "$tag" == "latest" ]]; then
...
# Enabled by setting SNIRELAY_FROM_SOURCE=1 in the environment.
build_from_source() {
    info "Building from source (SNIRELAY_FROM_SOURCE=1)..."
...
install_binary() {
    if [[ "${SNIRELAY_FROM_SOURCE:-0}" == "1" ]]; then
        build_from_source
    else
        download_binary || {
    ...
    cat > "${CONF_FILE}" <<'JSON'
{
  ...
  "LOG_LEVEL": "INFO",
  "LOG_FILE": "/var/log/sni-relay/service.log",
...
[Unit]
Description=sni-relay TCP proxy
...
show_stats() {
    ...
    while true; do
        clear
        printf '%s\n' "${C6}═══ sni-relay Stats — $(date '+%H:%M:%S') ═══${RST}"
...
backup_conf() {
    ...
    local dst="/root/sni-relay-backup-${ts}.tar.gz"
...
_banner_header() {
    ...
    # rainbow title box
    printf '%s\n' "${C1}╔══════════════════════════════════════════════════╗${RST}"
    printf '%s\n' "${C2}║${BOLD}${FG_WHITE}             sni-relay Manager${RST}${C2}║${RST}"
